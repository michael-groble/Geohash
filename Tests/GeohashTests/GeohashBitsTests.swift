...
class GeohashBitsTests: XCTestCase {
  ...
  func testEvenStringDecoding() throws {
    ...
    XCTAssertEqualWithAccuracy(bits.boundingBox().center().longitude,  0.0999999605119228, accuracy: 1.0e-13)
    XCTAssertEqualWithAccuracy(bits.boundingBox().center().latitude,  51.500000031665,     accuracy: 1.0e-13)
  }

  func testOddStringDecoding() throws {
    let bits = try GeohashBits(hash: "u10hfr2c4pv")
    XCTAssertEqualWithAccuracy(bits.boundingBox().center().longitude,   0.100000128149986, accuracy: 1.0e-13)
    XCTAssertEqualWithAccuracy(bits.boundingBox().center().latitude,   51.5000002831221,   accuracy: 1.0e-13)
  }
  ...
  func testEvenBinaryEncoding() throws {
    ...
    XCTAssertEqualWithAccuracy(bits.boundingBox().center().longitude, -0.10000079870223999, accuracy: 1.0e-13)
    XCTAssertEqualWithAccuracy(bits.boundingBox().center().latitude,  51.4999996125698,     accuracy: 1.0e-13)
  }

  func testOddBinaryEncoding() throws {
    let bits = try GeohashBits(location: Location(longitude: -0.1, latitude: 51.5), bitPrecision: 25)
    XCTAssertEqual(bits.hash(), "gcpuvxr1jz")
    XCTAssertEqualWithAccuracy(bits.boundingBox().center().longitude, -0.0999981164932251, accuracy: 1.0e-13)
